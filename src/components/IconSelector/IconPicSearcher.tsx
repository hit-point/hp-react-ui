...
const PicSearcher: React.FC = () => {
  ...
  const predict = (imgEl) => {
    try {
      ...
      let icons = window.antdIconClassifier.predict(imgEl) as any;
  ...
  const toImage = (url: string) => new Promise((resolve) => {
    const img = new Image();
  ...
  return (
    <div className='iconPicSearcher'>
      ...
      >
        ...
        <Spin spinning={state.loading} tip={formatMessage({ id: 'app.docs.components.icon.pic-searcher.matching' })}>
