          ...
          onClick={() => {
            getRoleMenuList(record.roleId).then((res) => {
              if (res.code === 200) {
                ...
                setMenuIds(res.checkedKeys.map((item) => `${item}`));
              ...
              if (key === 'datascope') {
                ...
                getDeptTreeSelect(record.roleId).then((resp) => {
                  if (resp.code === 200) {
                    ...
                    setMenuIds(resp.checkedKeys.map((item: number) => `${item}`));
                  }
                });
    ...
    <PageContainer>
      ...
      <div style={{ width: '100%', float: 'right' }}>
        <ProTable<API.System.Role>
          ...
          request={(params) => getRoleList({ ...params } as API.System.RoleListParams).then((res) => {
            const result = {
              data: res.rows,
              total: res.total,
              success: true,
