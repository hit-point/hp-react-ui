    ...
    <PageContainer>
      ...
      <Row gutter={[16, 24]}>
        ...
        <Col lg={18} md={24}>
          <ProTable<API.System.User>
            ...
            request={(params) => getUserList({ ...params, deptId: selectDept.id } as API.System.UserListParams).then((res) => {
              const result = {
                data: res.rows,
                total: res.total,
                success: true,
      ...
      <UpdateForm
        onSubmit={async(values) => {
          let success = false;
          if (values.userId || currentRow?.userId) {
            success = await handleUpdate({ ...values, userId: currentRow?.userId } as API.System.User);
          } else {
