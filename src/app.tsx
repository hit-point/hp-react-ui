  ...
  const fetchUserInfo = async () => {
    try {
      ...
      if (response.user.avatar === '') {
        response.user.avatar = '/icons/logoMini.png';
      }
      return {
        ...response.user,
...
export const layout: RunTimeLayoutConfig = ({ initialState, setInitialState }) => ({
  ...
  menu: {
    ...
    request: async () => {
      // console.log(getRemoteMenu(), 'getRemoteMenu()');
  ...
  pageTitleRender: (props) => props.pageName as any,
  // logo: <img style={{ width: '170px', height: '43px' }} src='/icons/byLogo.png' />,
  ...
  subMenuItemRender: (subMenuItemProps, defaultDom) => {
    if (subMenuItemProps.isUrl || !subMenuItemProps.path) {
      return defaultDom;
    }
    // 支持二级sub菜单显示icon
    return (
      <div style={{ display: 'flex', gap: '8px', flexDirection: 'row', alignItems: 'center' }}>
        {subMenuItemProps.pro_layout_parentKeys &&
          subMenuItemProps.pro_layout_parentKeys.length > 0 &&
          subMenuItemProps.icon}
        {defaultDom}
      </div>
    );
  },
